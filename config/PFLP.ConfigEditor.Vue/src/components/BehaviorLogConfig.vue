<template>
  <div>
    <n-text type="info">
      数据库格式SQLite<br />
      推荐使用<a href="https://www.sqlitestudio.pl/" target="_blank"
        >SQLiteStudio</a
      >查看<br /><br />
    </n-text>
    <n-space>
    ...
    <div v-if="beh.ModuleEnabled">
      ...
      <n-space>
        日志刷新间隔<n-input-number
          v-model:value="beh.RefreshIntervalInMinutes"
          size="large"
        />分钟
      </n-space>
      <div>
        <n-space v-for="(item, key) in beh.Toggles">
          {{ getLogName(key) }} ： 到数据库<n-switch
            v-model:value="item.Database"
            size="large"
          />
