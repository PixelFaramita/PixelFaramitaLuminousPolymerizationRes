<template>
  <div>
    ...
    <div v-if="motdx.ModuleEnabled">
      <n-space>
        自动最大玩家数量（=当前玩家数量+1）:<n-switch
          v-model:value="motdx.DynamicMaxCount"
        />
      </n-space>
      <n-space>
        伪造在线玩家数 (0为关闭)：
        <n-input-number v-model:value="motdx.FakeCurrentPlayerCount" />
      </n-space>
      <n-space>
        修改最大玩家数 (0为关闭)：
        <n-input-number v-model:value="motdx.FakeMaxPlayerCount" />
      </n-space>
      <n-space>
        伪造motd显示端口(IPv4) (0为关闭)：
        <n-input-number
          :validator="(val) => val <= 65535 && val >= 0"
          v-model:value="motdx.FakePortIPv4"
        />
      </n-space>
      <n-space>
        伪造motd显示端口(IPv6) (0为关闭)：
        <n-input-number
          :validator="(val) => val <= 65535 && val >= 0"
      ...
      <n-space>
        修改motd存档名称(留空关闭)：
        <n-input v-model:value="motdx.FakeLevelName" />
      </n-space>
    </div>
    <n-h3 prefix="bar">
      攻击回显
    ...
    <div v-if="attack.ModuleEnabled">
      ...
      <n-space>
        弹射物击中物品栏小字显示伤害:<n-switch
          v-model:value="attack.ShowActionbar"
        />
      </n-space>
    </div>
    <n-h3 prefix="bar">
    ...
    <div v-if="helper.ModuleEnabled">
      ...
      <n-space>
        ...
        <n-a
          v-if="helper.EditRedstoneTPS"
          href="https://wiki.biligame.com/mc/%E5%88%BB"
          >刻/TPS/MSPT参考</n-a
        >
      </n-space>
      <n-space v-if="helper.EditRedstoneTPS">
        <n-space>
          ...
          <n-input-number
            step="0.5"
            v-model:value="helper.OverworldRedstoneTPS"
          >
            <template #suffix> TPS </template>
          </n-input-number>
          <n-space>
            =<n-text class="blue">{{
              TpsToMSPT(helper.OverworldRedstoneTPS)
            }}</n-text>
        ...
        <n-space>
          ...
          <n-space>
            =<n-text class="blue">{{
              TpsToMSPT(helper.NetherRedstoneTPS)
            }}</n-text>
            <n-text class="yellow">MSPT</n-text>
            =<n-text class="yellow">原速</n-text>÷<n-text class="blue">{{
        ...
        <n-space>
          ...
          <n-space>
            =<n-text class="blue">{{
              TpsToMSPT(helper.TheEndRedstoneTPS)
            }}</n-text>
            <n-text class="yellow">MSPT</n-text>
            =<n-text class="yellow">原速</n-text>÷<n-text class="blue">{{
    ...
    <div v-if="height.ModuleEnabled">
      <n-space>
        ...
        <n-input-number
          step="16"
          v-model:value="height.Max"
          :validator="(x: number) => x <= 512"
        />
      </n-space>
      <n-space>
        最低高度
    ...
    <div v-if="close.TransferServerOnClose">
      ...
      <n-space>
        ...
        <n-input-number
          step="16"
          v-model:value="close.Port"
          :validator="(x: number) => x >= 0 && x <= 65535"
        />
      </n-space>
    </div>
    <n-divider />
    ...
    <div v-if="disconnect.ModuleEnabled">
      ...
      <n-space>
        强制允许的协议版本
        <n-dynamic-input
          v-model:value="disconnect.ExtraAllowIncomingProtocolVersions"
          :on-update-value="() => {
...
export default {
  ...
  methods: {
    ...
    TpsToCount: function (val: number) {
      return Math.max(Math.round(40 / val) / 2, 0.5);
    },
  },
};
</script>
