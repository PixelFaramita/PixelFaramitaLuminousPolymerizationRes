  ...
  <div>
    <n-space>
      ...
      <n-select
        ...
        v-model:value="config.ConsoleLanguage"
        placeholder="语言"
        :options="options"
        :filterable="true"
      ...
      <n-input
        ...
        placeholder="语言"
      />
      <n-switch v-model:value="selected" size="large">
        <template #checked> 选择 </template>
        <template #unchecked> 自定义 </template>
      </n-switch>
    </n-space>
    <n-space>
      全局表单每页个数<n-input-number
        v-model:value="config.FormAPI_CountPerPage"
      />
    </n-space>
    <n-space>
      全局传送免伤时间<n-input-number
        step="500"
        v-model:value="config.Global_AsyncTeleportWaitTime"
      >
    ...
    <n-space>
      依赖材质包的图形粒子
      <n-switch
        v-model:value="config.ParticlesGraphics_WithParticlesResourcePack"
        size="large"
      >
    ...
    <n-space>
      材质包加密
      <n-switch
        v-model:value="config.PackManager_EncryptResourcePacks"
        size="large"
      >
        <template #checked> 开启 </template>
        <template #unchecked> 关闭 </template>
    ...
    <n-space>
      用于打开配置UI的控制台命令
      <n-input
        v-model:value="config.WebViewConfig_ConsoleCommand"
        size="large"
      />（基于<a
  ...
  computed: {
    options() {
      return [
        ...
        { value: "id_ID", label: "Indonesia (Indonesia)" },
        // "nb_NO" => "Norsk bokmål (Norge)",
        { value: "nb_NO", label: "Norsk bokmål (Norge)" },
        // "pl_PL" => "Polski (Polska)",
